function varargout=rs_imana_new_1117(what,varargin)
...
switch(what)
    ...
    case 'preprocess_comparison'
        % still to do:
        % create figure out of this
        % do not limited it to M1 or should I?
        type = {'pre','preproc'};
    ...
    case 'NC_getInterHemPattern'        % get pattern of correlations for pairs between hemispheres
        % 0. Load data
        D       = rs_imana_new_1117('PP_studySubset');
        D       = rs_imana_new_1117('PP_removePair',D);
...
%           % anovaMixed(S.r,sn,'between',S.control,{'grp'});
        ...
        figure;
          ...
          barplot(S.weekLabel,S.r,'plotfcn','fisherinv(nanmean(x))','split',S.control,'subset', S.control ==1,'CAT',s(1).CAT,s(1).PLOT{:});
          set_graphics(gcf,'ylabel',{'correlation'}, 'xlabel',{'weeks'},'fontsize_all',14);
          title(fName(i)); 
          ylim([0 0.61]);
          % [~,~,sn] = unique(S.subj_name);
        ...
        figure;
          subplot(2,2,i)
          s   = style_sheet(sty_2grp_cp,'leg',{'control','patient'},'leglocation','northoutside','errorcolor',sty_2grp_cp);
         
          barplot(S.weekLabel,S.r,'plotfcn','fisherinv(nanmean(x))','split',S.control,'subset', S.control ==0,'CAT',s(1).CAT,s(1).PLOT{:});
          set_graphics(gcf,'ylabel',{'correlation'}, 'xlabel',{'weeks'},'fontsize_all',14);
          title(fName(i)); 
          ylim([0 0.61]);
          % [~,~,sn] = unique(S.subj_name);
   ...
   keyboard;     
    case 'REL_withinAll'                % calculate within subject correlations (early/late after stroke)
        ...
        for i=1:length(fName)
            D = load(fullfile([ppDir],sprintf('%s.mat',fName{i})));
            D = rs_imana_new_1117('REL_withinSubj',D);
...
%         
        ...
        barplot([S.week],S.sh_fz_r,'split',S.control,'plotfcn','fisherinv(nanmean(x))','CAT',s(1).CAT,s(1).PLOT{:});
...
%         
    ...
    case 'FIG_all'    
        ...
        subplot(221);
        rs_imana_new_1117('FIG_inter_splithalf_reliability');
        ylim([0 1]);
        ylabel('Fisher Z')
        xlabel('weeks')
        title('Interhemispheric')
        subplot(222);
        rs_imana_new_1117('FIG_intraLesioned_splithalf_reliability');
        ylim([0 1]);
        ylabel('Fisher Z')
        xlabel('weeks')
        title('Intrahemispheric Les')
        subplot(223);
        rs_imana_new_1117('FIG_intraNonlesioned_splithalf_reliability');
        ylim([0 1]); 
        ylabel('Fisher Z')
        xlabel('weeks')
        title('Intrahemispheric NonLes')
        subplot(224);
        rs_imana_new_1117('FIG_splithalf_all');
        ylim([0 1]); 
        ylabel('Fisher Z')
        xlabel('weeks')
...
%         end
    ...
    case 'MainFigure_All'
        subplot(221);
        rs_imana_new_1117('MainFigure_Inter');
        ylim([0.5 1.1]);
        ylabel('Fisher Z')
        xlabel('weeks')
        subplot(222);
        rs_imana_new_1117('MainFigure_Intra_Lesioned');
        ylim([0.5 1.1]);
         ylabel('Fisher Z')
        xlabel('weeks')
        subplot(223);
        rs_imana_new_1117('MainFigure_Intra_NonLesioned');
        ylim([0.5 1.1]); 
        ylabel('Fisher Z')
        xlabel('weeks')        
        subplot(224);
        rs_imana_new_1117('MainFigure_all_conn');
        ylim([0.5 1.1]); 
        ylabel('Fisher Z')
        xlabel('weeks')   
    ...
    case 'MainFigure_EUC'
        subplot(221);
        rs_imana_new_1117('MainFigure_Inter_EUC');
        ylim([0.8 2.2]);
        ylabel('Euclidian distance')
        xlabel('weeks')
        subplot(222);
        rs_imana_new_1117('MainFigure_Intra_Lesioned_EUC');
        ylim([0.8 2.2]);
         ylabel('Euclidian distance')
        xlabel('weeks')
        subplot(223);
        rs_imana_new_1117('MainFigure_Intra_NonLesioned_EUC');
        ylim([0.8 2.2]); 
        ylabel('Euclidian distance')
        xlabel('weeks')        
        subplot(224);
        rs_imana_new_1117('MainFigure_all_conn_EUC');
        ylim([0.8 2.2]); 
        ylabel('Euclidian distance')
        xlabel('weeks')        
        keyboard;
    ...
    case 'MainFigure_EUC_patients' 
        subplot(221);
        rs_imana_new_1117('MainFigure_Inter_EUC_patients');
        ylim([0.8 2.2]);
        ylabel('Euclidian distance')
        xlabel('weeks')
        subplot(222);
        rs_imana_new_1117('MainFigure_Intra_Lesioned_EUC_patients');
        ylim([0.8 2.2]);
         ylabel('Euclidian distance')
        xlabel('weeks')
        subplot(223);
        rs_imana_new_1117('MainFigure_Intra_NonLesioned_EUC_patients');
        ylim([0.8 2.2]); 
        ylabel('Euclidian distance')
        xlabel('weeks')        
        subplot(224);
        rs_imana_new_1117('MainFigure_all_conn_EUC_patients');
        ylim([0.8 2.2]); 
        ylabel('Euclidian distance')
        xlabel('weeks')        
        keyboard;
...
%         D   = load(fullfile(rootDir,'rs_networkCorr.mat'));                
    ...
    case 'plot_networkCorr_nonlesioned_cortical' % plot controls and patients (non lesioned hemisphere only)
        D       = load(fullfile(rootDir,'rs_networkCorr_nonlesionedIntra.mat'));        
        D       = rs_imana_new_1117('PP_removeInf',D);   
        %take only subgroup
        D   = getrow(D,(D.lesionType~=3)); %no subcortical
