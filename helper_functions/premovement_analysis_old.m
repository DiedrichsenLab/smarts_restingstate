...
switch (what)
    ...
    case 'preprocess_combine'                % Gets each subject's data summary
        ...
        for i = sn
            ...
            if exist(IHIRfile,'file')
                ...
                R.paretic_side   = repmat(S.paretic_side(i),length(R.BN),1);
                R.FM_arm         = repmat(S.FM_arm(i),length(R.BN),1);
                R.FM_wrist       = repmat(S.FM_wrist(i),length(R.BN),1);
                R.FM_hand        = repmat(S.FM_hand(i),length(R.BN),1);
                R.FM_total       = repmat(S.FM_total(i),length(R.BN),1);
                R.MVC_FDI_par    = repmat(S.MVC_FDI_par(i),length(R.BN),1);
                R.MVC_FDI_unpar  = repmat(S.MVC_FDI_unpar(i),length(R.BN),1);
                R.ARAT_par       = repmat(S.ARAT_par(i),length(R.BN),1);
            ...
            D.paretic_side   = repmat(S.paretic_side(i),length(D.BN),1);
            D.FM_arm         = repmat(S.FM_arm(i),length(D.BN),1);
            D.FM_wrist       = repmat(S.FM_wrist(i),length(D.BN),1);
            D.FM_hand        = repmat(S.FM_hand(i),length(D.BN),1);
            D.FM_total       = repmat(S.FM_total(i),length(D.BN),1);
            D.MVC_FDI_par    = repmat(S.MVC_FDI_par(i),length(D.BN),1);
            D.MVC_FDI_unpar  = repmat(S.MVC_FDI_unpar(i),length(D.BN),1);
            D.ARAT_par       = repmat(S.ARAT_par(i),length(D.BN),1);
    ...
    case 'make_summary_tms_scheduled'% Averages individual subject's data by 4 scheduled TMS timings
        ...
        T = tapply(D,{'ID','subj_name','SubjN','week','control','lesionside','tms_schedule','state','task'},...
                   {'pre_gort'},...
                   {'mep'},...
    ...
    case 'make_summary_real_timing'  % Averages individual subject's data by real TMS timings binned to severl categories
        D = load(fullfile(analysisDir,'alldat.mat'));
        D = getrow(D,D.good_stim==1 &~(~D.isGood==1 &((D.RT>0&D.pre_act>0.1)|D.RT>1000)));
    ...
    case 'plot_tms_timing_individual'% plots singl subject mean & scatter plot of a variable for single vs. conditioned pulses
        ...
        for w=1:length(weeks)
            ...
            if strcmp(var,'mep')
                ...
                myboxplot(D.(timing_var),D.(var),'style_tukey','plotall',2,'split',D.state,...
                          'subset',(D.week==weeks(w)&ismember(D.subj_name,subj)&D.isGood&D.task==2),...
                          'leg',legs,'leglocation','northwest','xtickoff');
    ...
    case 'plot_tms_timing'           % Plots group mean of a variable (e.g. mep, RT) for single vs. conditioned pulses at each TMS timing
        ...
        DD = getrow(D,(D.mep_count>=count&D.control==control&ismember(D.subj_name,subj)&D.task==2));
        figure;
        legs = {'un-conditioned','conditioned'};
        for w=1:length(weeks)
    ...
    case 'correlate_mep_rt'          % correlates MEP size and RT for single vs. paired pulses
        var1    = varargin{1};
        var2    = varargin{2};
        timing  = 'scheduled';  % 'scheduled' vs. 'real'
        control = 0;
