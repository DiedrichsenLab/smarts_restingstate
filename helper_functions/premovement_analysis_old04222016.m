...
color_t = {[58 87 191]./255,[19 56 192]./255,[8 32 121]./255,[4 23 92]./255};
switch (what)
    ...
    case 'preprocess_combine'                % Gets each subject's data summary
        ...
        for i = sn
            ...
            if exist(IHIRfile,'file')
                IHIR = load(IHIRfile);
                R.state          = double(IHIR.Summary.channel(1).state);
                ...
                R.control        = logical(repmat(~S.isPatient(i),length(R.BN),1));
                R.age            = repmat(S.age(i),length(R.BN),1);
                R.gender         = repmat(S.gender(i),length(R.BN),1);
                R.handedness     = repmat(S.handedness(i),length(R.BN),1);
                R.isGood         = logical(IHIR.Summary.channel(1).isGood & IHIR.Summary.channel(2).isGood);
                ...
                R.tms_schedule   = zeros(length(R.state),1);  % TMS timing as scheduled
                R.tms_schedule_r = zeros(length(R.state),1); % TMS timing in the real trial
                R.CS_intensity   = repmat(S.CS_intensity_rest(i),length(R.BN),1);
                R.CS_intensity(R.state==1) = NaN;
                R.TS_intensity   = repmat(S.TS_intensity_rest(i),length(R.BN),1);
                R.rCST           = repmat(S.rCST(i),length(R.BN),1);
                R.aCST           = repmat(S.aCST(i),length(R.BN),1);
                R.task           = ones(length(R.BN),1); % 1=IHIR
                R.FMA            = repmat(S.FMA(i),length(R.BN),1);
                R.FMW            = repmat(S.FMW(i),length(R.BN),1);
                R.FMH            = repmat(S.FMH(i),length(R.BN),1);
                R.FM_total       = repmat(S.FM_total(i),length(R.BN),1);
                R.DynFDI_paretic = repmat(S.DynFDI_paretic(i),length(R.BN),1);
                R.DynFDI_nparetic= repmat(S.DynFDI_nparetic(i),length(R.BN),1);
                R.ARAT           = repmat(S.ARAT(i),length(R.BN),1);
            else
                disp([S.code2{i} ': no IHIR']);
            end
            ...
            D.BN             = BN;
            D.TN             = task.tms.TN;
            D.control        = logical(repmat(~S.isPatient(i),length(D.BN),1));
            D.age            = repmat(S.age(i),length(D.BN),1);
            D.gender         = repmat(S.gender(i),length(D.BN),1);
            D.handedness     = repmat(S.handedness(i),length(D.BN),1);
            D.lesionside     = repmat(S.lesionside(i),length(D.BN),1);
            D.subj_name      = repmat({S.name{i}},length(D.BN),1);
            D.good_stim      = logical(task.tms.good_stim);
            D.isGood         = logical(task.tms.good_stim & isGood);
            D.mep            = mep;
            D.pre_gort       = repmat(task.tms.pre_gort,length(D.BN),1); % Go RT tested before TMS session
            D.ts_time        = (task.tms.scheduled).*task.tms.pre_gort; % test pulse relative to Go cue onset
            ...
            D.task           = 2*ones(length(D.BN),1); % 2=IHI-Premove
            D.FMA            = repmat(S.FMA(i),length(D.BN),1);
            D.FMW            = repmat(S.FMW(i),length(D.BN),1);
            D.FMH            = repmat(S.FMH(i),length(D.BN),1);
            D.FM_total       = repmat(S.FM_total(i),length(D.BN),1);
            D.DynFDI_paretic = repmat(S.DynFDI_paretic(i),length(D.BN),1);
            D.DynFDI_nparetic= repmat(S.DynFDI_nparetic(i),length(D.BN),1);
            D.ARAT           = repmat(S.ARAT(i),length(D.BN),1);
    ...
    case 'make_alldat'               % Makes the alldata structure: Do this before calling other tabs
        ...
        for i = sn
            ...
            D.Centre = repmat({S.Centre{i}},length(D.BN),1);
    ...
    case 'make_summary_tms_scheduled'% Averages individual subject's data by 4 scheduled TMS timings
        ...
        DD = getrow(D,D.isGood==1);
        T = tapply(DD,{'ID','subj_name','SubjN','week','control','lesionside','tms_schedule','state','task'},...
                   {'age'},...
                   {'gender'},...
                   {'pre_gort'},...
                   {'handedness'},...
        ...
        % taking out outliers outside 2std
        % 1. label each trial in alldat
        subj = unique(D.SubjN);
        for s=subj'
            for w=weeks'
                for t=tms_time'
                    for st=(1:2)
                        mep_cutoff1 = T.mep(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st)+...
                                      2*T.mep_std(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st);
                        mep_cutoff2 = T.mep(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st)-...
                                      2*T.mep_std(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st);
                        D.mep_outlier(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)=...
                            (D.mep(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)>mep_cutoff1|...
                             D.mep(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)<mep_cutoff2);
                        rt_cutoff1 = T.RT(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st)+...
                                      2*T.RT_std(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st);
                        rt_cutoff2 = T.RT(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st)-...
                                      2*T.RT_std(T.SubjN==s&T.week==w&T.tms_schedule==t&T.state==st);
                        D.RT_outlier(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)=...
                            (D.RT(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)>rt_cutoff1|...
                             D.RT(D.SubjN==s&D.week==w&D.tms_schedule==t&D.state==st)<rt_cutoff2);
                    end
                end
            end
        end
        if size(D.mep_outlier,1) == 1
            D.mep_outlier = D.mep_outlier';
        end
        if size(D.RT_outlier,1) == 1
            D.RT_outlier = D.RT_outlier';
        end        
        DP = getrow(D,D.control==0);
        DC = getrow(D,D.control==1);
        save(fullfile(analysisDir,'alldat.mat'),'-struct','D');
        dsave(fullfile(analysisDir,'alldat.dat'),D);
        save(fullfile(analysisDir,'alldat_patient.mat'),'-struct','DP');
        save(fullfile(analysisDir,'alldat_control.mat'),'-struct','DC');
        
        % 2. take out outliers in new summary
        DD = getrow(D,D.isGood==1&D.mep_outlier==0);
        T = tapply(DD,{'ID','subj_name','SubjN','week','control','lesionside','tms_schedule','state','task'},...
                   {'age'},...
                   {'gender'},...
                   {'pre_gort'},...
                   ...
                   {'DynFDI_paretic'},...
                   {'DynFDI_nparetic'});
        save(fullfile(analysisDir,'summary_tms_scheduled_mepoutlier.mat'),'-struct','T');
        dsave(fullfile(analysisDir,'summary_tms_scheduled_mepoutlier.dat'),T);
        
        DD = getrow(D,D.isGood==1&D.RT_outlier==0);
        T = tapply(DD,{'ID','subj_name','SubjN','week','control','lesionside','tms_schedule','state','task'},...
                   {'age'},...
                   {'gender'},...
                   {'pre_gort'},...
                   ...
                   {'DynFDI_paretic'},...
                   {'DynFDI_nparetic'});
        save(fullfile(analysisDir,'summary_tms_scheduled_rtoutlier.mat'),'-struct','T');
        dsave(fullfile(analysisDir,'summary_tms_scheduled_rtoutlier.dat'),T);
    ...
    case 'make_summary_real_timing'  % Averages individual subject's data by real TMS timings binned to severl categories
        D = load(fullfile(analysisDir,'alldat.mat'));
        D = getrow(D,D.good_stim~=0&~(~D.isGood==1&((D.RT>0&D.pre_act>0.1)|D.RT>1000)));
        ...
        T = tapply(D,{'ID','subj_name','SubjN','week','control','lesionside','tms_bin','state','task'},...
                   {'age'},...
                   {'gender'},...
                   {'pre_gort'},...
                   {'handedness'},...
    ...
    case 'plot_tms_timing_individual'% plots singl subject mean & scatter plot of a variable for single vs. conditioned pulses
        ...
        rmoutlier = 0; % 2 standard deviation out
        vararginoptions(varargin(3:end),{'timing','rmoutlier'});
        D = load(fullfile(analysisDir,'alldat.mat'));
            ...
            if strcmp(var,'mep')
                if rmoutlier == 0
                    myboxplot(D.(timing_var),D.(var),'style_tukey','plotall',2,'split',D.state,...
                              'subset',(D.week==weeks(w)&ismember(D.subj_name,subj)&D.isGood==1&D.mep_outlier==0),...
                              'leg',legs,'leglocation','northwest','xtickoff');
                else
                    myboxplot(D.(timing_var),D.(var),'style_tukey','plotall',2,'split',D.state,...
                              'subset',(D.week==weeks(w)&ismember(D.subj_name,subj)&D.isGood==1),...
                              'leg',legs,'leglocation','northwest','xtickoff');
                end
                rcst = nanmean(D.rCST(strcmp(D.subj_name,subj)&D.isGood==1&D.week==weeks(w)));
                acst = nanmean(D.aCST(strcmp(D.subj_name,subj)&D.isGood==1&D.week==weeks(w)));
                drawline(rcst,'dir','horz','color','b');
                drawline(acst,'dir','horz','color','r');
                set(gca,'XTickLabel',{'R','','0.2','','0.5','','0.8','','0.95',''})
                ylim([-0.1 8]);

            else
                if rmoutlier==0
                    myboxplot(D.(timing_var),D.(var),'style_tukey','plotall',2,'split',D.state,...
                              'subset',(D.week==weeks(w)&ismember(D.subj_name,subj)&D.isGood==1&D.task==2&D.RT_outlier==0),...
                              'leg',legs,'leglocation','northwest','xtickoff');
                else
                    myboxplot(D.(timing_var),D.(var),'style_tukey','plotall',2,'split',D.state,...
                              'subset',(D.week==weeks(w)&ismember(D.subj_name,subj)&D.isGood==1&D.task==2),...
                              'leg',legs,'leglocation','northwest','xtickoff');
                end
                set(gca,'XTickLabel',{'0.2','','0.5','','0.8','','0.95',''})
                ylim([120 700]);
            end
    ...
    case 'plot_tms_timing'           % Plots group mean of a variable (e.g. mep, RT) for single vs. conditioned pulses at each TMS timing
        ...
        timing  = 'scheduled';  % 'scheduled' vs. 'real'
        rmoutlier = 0; % 2 standard deviation out
        vararginoptions(varargin(2:end),{'subj','mepcount','control','timing','rmoutlier'});
        if strcmp(timing,'scheduled')
            if rmoutlier
                D = load(fullfile(analysisDir,['summary_tms_scheduled_' var 'outlier.mat']));
            else
        ...
        DD = getrow(D,(D.mep_count>=mepcount&D.control==control&ismember(D.subj_name,subj)));
        figure;
        legs = {'un-conditioned','conditioned'};
        for w=1:length(weeks)
            [f1,r1,c1] = pivottable(DD.SubjN,DD.(timing_var),DD.(var),'nanmean',...
                'subset',DD.week==weeks(w)&DD.state==1);
            [f2,r2,c2] = pivottable(DD.SubjN,DD.(timing_var),DD.(var),'nanmean',...
                'subset',DD.week==weeks(w)&DD.state==2);
            [preGoRT,rr1,cc1]= pivottable(DD.SubjN,DD.control,DD.pre_gort,'nanmean','subset',DD.week==weeks(w));
            [rcst,rr2,cc2]   = pivottable(DD.SubjN,DD.control,DD.rCST,'nanmean','subset',DD.week==weeks(w));
            [acst,rr3,cc3]   = pivottable(DD.SubjN,DD.control,DD.aCST,'nanmean','subset',DD.week==weeks(w));
            % check number of subjects in each TMS timing
            nlabel   = [];
            tms_time = intersect(c1,c2);
            states   = repmat([1;2],length(tms_time),1);
            subjs    = intersect(r1,r2);
            var_mean = [];
            var_err  = [];
            for t=1:length(tms_time)
                idx{t}   = intersect(find(~isnan(f1(ismember(r1,subjs),(c1==tms_time(t))))),...
                                     find(~isnan(f2(ismember(r2,subjs),(c2==tms_time(t))))));
                n(t)     = length(idx{t});
                nlabel   = [nlabel,{['N=' num2str(n(t))]}];
                var_mean = [var_mean; [mean(f1(idx{t},(c1==tms_time(t))));mean(f2(idx{t},(c2==tms_time(t))))]];
                var_err  = [var_err [stderr(f1(idx{t},(c1==tms_time(t)))) stderr(f2(idx{t},(c2==tms_time(t))))]];
            end
            subplot(2,3,w);
            ts = repmat(tms_time,1,2)';
            xcoord = barplot(ts(:),var_mean,'split',states,'errorfcn',var_err,'leg',legs);
            if strcmp(var,'RT');
                drawline(nanmean(preGoRT),'dir','horz','color','r');  % draw a line for pre_gort
            end
            if strcmp(var,'mep')
                drawline(nanmean(rcst),'dir','horz','color','b');  % draw a line for rCST
                drawline(nanmean(acst),'dir','horz','color','r');  % draw a line for aCST                
            end
            if strcmp(timing,'scheduled')
                if strcmp(var,'RT')
                    if control ==0
    ...
    case 'correlate_vars'          % correlates MEP size and RT for single vs. paired pulses
        var1    = varargin{1};  %'mep'
        var2    = varargin{2};  %'RT'
        timing  = 'scheduled';  %'scheduled' vs. 'real'
        control = 0;
        vararginoptions(varargin(3:end),{'timing','control'});
        D = load(fullfile(analysisDir,'alldat.mat'));
        if strcmp(timing,'scheduled')
            timing_var = 'tms_schedule';
        elseif strcmp(timing,'real')
        ...
        D = getrow(D,D.isGood&D.task==2&D.control==control);
        [r,p] = corrcoef(D.(var1)(D.state==1),D.(var2)(D.state==1),'rows','pairwise');
        Crr(1) = r(1,2);
        P(1)   = p(1,2);
        [r,p] = corrcoef(D.(var1)(D.state==2),D.(var2)(D.state==2),'rows','pairwise');
        Crr(2) = r(1,2);
        P(2)   = p(1,2);
        
        [dat1,row1,col1] = pivottable(D.SubjN,D.state,D.(var1),'nanmean');
        [dat2,row2,col2] = pivottable(D.SubjN,D.state,D.(var2),'nanmean');
        [r,p] = corrcoef(dat1(:,1),dat2(:,1),'rows','pairwise');
        Crr_s(1) = r(1,2);
        P_s(1)   = p(1,2);
        [r,p] = corrcoef(dat1(:,2),dat2(:,2),'rows','pairwise');
        Crr_s(2) = r(1,2);
        P_s(2)   = p(1,2);
        [r,p] = corrcoef((dat1(:,1)-dat1(:,2)),(dat2(:,1)-dat2(:,2)),'rows','pairwise');
        Crr_s(3) = r(1,2);
        P_s(3)   = p(1,2);
        
        figure;
        subplot(1,3,1);
        scatterplot(dat1(:,1),dat2(:,1),'regression','linear',...
            'markersize',5,'markercolor','k','markerfill','k','printcorr');
        xlabel(var1,'interpreter','none','fontsize',12);
        ylabel(var2,'interpreter','none','fontsize',12);
        title('State 1','fontsize',12);
        axis square;
        subplot(1,3,2);
        scatterplot(dat1(:,2),dat2(:,2),'regression','linear',...
            'markersize',5,'markercolor','k','markerfill','k','printcorr');
        xlabel(var1,'interpreter','none','fontsize',12);
        ylabel(var2,'interpreter','none','fontsize',12);
        title('State 2','fontsize',12);
        axis square;        
        
        subplot(1,3,3);
        scatterplot((dat1(:,1)-dat1(:,2)),(dat2(:,1)-dat2(:,2)),'regression','linear',...
            'markersize',5,'markercolor','k','markerfill','k','printcorr');
        xlabel(var1,'interpreter','none','fontsize',12);
        ylabel(var2,'interpreter','none','fontsize',12);
        title('State 1-2','fontsize',12);
        axis square;
                
        % split by tms_timing
        figure;
        timing = unique(D.(timing_var))';
        for t=1:length(timing)
            [dat1,row1,col1] = pivottable(D.SubjN,D.state,D.(var1),'nanmean','subset',D.(timing_var)==timing(t));
            [dat2,row2,col2] = pivottable(D.SubjN,D.state,D.(var2),'nanmean','subset',D.(timing_var)==timing(t));
            [r,p] = corrcoef(dat1(:,1),dat2(:,1),'rows','pairwise');
            Crr_s(1) = r(1,2);
            P_s(1)   = p(1,2);
            [r,p] = corrcoef(dat1(:,2),dat2(:,2),'rows','pairwise');
            ...
            scatterplot(dat1(:,1),dat2(:,1),'regression','linear',...
                'markersize',5,'markercolor',color_t{t},'markerfill',color_t{t},'printcorr');
            xlabel(var1,'interpreter','none','fontsize',12);
            ylabel(var2,'interpreter','none','fontsize',12);
            title(['State 1; Timing ' num2str(t)],'fontsize',12);
            axis square;
            ...
            xlabel(var1,'interpreter','none','fontsize',12);
            ylabel(var2,'interpreter','none','fontsize',12);
            title(['State 2; Timing ' num2str(t)],'fontsize',12);
            axis square;        
            ...
            xlabel(var1,'interpreter','none','fontsize',12);
            ylabel(var2,'interpreter','none','fontsize',12);
            title(['State 1-2; Timing ' num2str(t)],'fontsize',12);
            axis square;
        end
    ...
    case 'plot_IHI_curve_individual'
        ihi_var = varargin{1}; % 'perc' or 'ratio'
        subj    = {};
        count   = 9;
        timing  = 'scheduled';  % 'scheduled' vs. 'real'
        ...
        for i=1:length(subj)
            for w=1:length(weeks)
                for t=1:length(tms)
                    SESS = getrow(D,strcmp(D.subj_name,subj{i}) & D.(timing_var)==tms(t) &...
                                  D.week==weeks(w));
                    if ~isempty(SESS.ID)
        ...
        for w=1:length(weeks)
            ...
            subplot(2,3,w); hold on;
            plot(c(2:end),f(2:end)','linewidth',2);
            plot(c(1),f(1),'r*');
            ...
            for w=1:length(weeks)
                for t=1:length(tms)
                    SESS = getrow(D,strcmp(D.subj_name,subj{i}) &...
                                  D.(timing_var)==tms(t) & D.week==weeks(w) & D.task==2);
                    if ~isempty(SESS.ID)
                        % un-conditioned test pulse
                        UT_mep       = SESS.mep(SESS.state==1);
                        UT_mep_count = SESS.mep_count(SESS.state==1);
