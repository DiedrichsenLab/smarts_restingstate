function varargout = process_mep_IHI(dd,flags,manual,showfig)

% USAGE: [Summary,MEPs,flags] = process_mep_premove(dd,flags,manual,showfig,loadprocessed);
%
% INPUT:
% dd -- can be either a filename (full path), a datastructure
...
%
% flags -- a structure for analysis flags.  If not supplied, a
...
%           local minima/maxima to the cursor
%  Middle mouse button -- Mark reaction time
%
% manual -- a flag for marking the data manually or not
%           If 0, automatically chack good/bad, taking the current MEP peak&trough, leaving RT empty
%           Can only be used for situations when RT doesn't matter.
%
% OUTPUT:
...
% parameters for Vitznau with 312ms emgDelay due to digital EMG input
% emgDelay = 312;
% optflags = struct('channel',[3 4],'multiplier',1,'background_noise',1,...
%                   'mepTime',[1850:3000]+emgDelay,'preTime',[1850:2000]+emgDelay,...
%                   'totalTime',[1:5000],'gain',[],'frames',[]);

% for IHIR
%optflags = struct('channel',[3 4],'multiplier',1,'background_noise',1,...
%                  'mepTime',[850:1500],'preTime',[850:1000],'emgDelay',0,...
%                  'totalTime',[1:5000],'gain',[],'frames',[],'loadProcessed',1);

% parameters for UZ (University Hospital)
% emgDelay = 320;
...
emgDelay         = optflags.emgDelay;
mepTime          = optflags.mepTime+emgDelay;
totalTime        = optflags.totalTime;
preTime          = optflags.preTime+emgDelay;
MEP_Total_Offset = preTime(1);
loadProcessed    = optflags.loadProcessed;
...
if exist('file','var')
    ...
    if exist(mep_file,'file')&&loadProcessed
        load(mep_file);
    end
end
...
while i <= numFrames
    ...
    % automatically preprocessing: taking the current MEP peak&trough, leaving RT empty for now
    % Guess MEP min and max of each trial
    for c=1:length(data_channel)
        startT = preTime(end)-preTime(1)+10;
        [D.channel(c).peak,D.channel(c).peakTime] = max(M.channel(c).trace{1}(startT:startT+50));
        D.channel(c).peakTime = D.channel(c).peakTime+startT; % start from time of TS pulse
        [D.channel(c).trough,D.channel(c).troughTime] = min(M.channel(c).trace{1}(startT:startT+50));
        D.channel(c).troughTime = D.channel(c).troughTime+startT;
    ...
    if background_noise
        for c=1:length(data_channel)
            baseline = mean(dd.values(preTime,data_channel(c),f));  % in case EMG signals are not 0-centered
            D.channel(c).rms = sqrt(mean((dd.values(preTime,data_channel(c),f)-baseline).^2));
            D.channel(c).pre_act_trial_avg = mean(abs((dd.values(preTime, ...
                                                    data_channel(c),f)-baseline)));
            D.channel(c).pre_act_trial_std = std(abs((dd.values(preTime, ...
                                                    data_channel(c),f)-baseline)));
            M.channel(c).bg = {squeeze((dd.values(preTime,data_channel(c),f)-baseline))};
            M.channel(c).bg_rec = {abs(M.channel(c).bg{1})};
            if ~manual
                if c==1
                    D.channel(c).isGood = (D.channel(c).amplitude>0.05 & D.channel(c).rms<0.02);
                else
                    D.channel(c).isGood = D.channel(c).rms<0.02;
                end
    ...
    if showfig
        ...
        for c=1:length(data_channel)
            ...
            line(XL,[-0.05 -0.05],'Color','k');
            line([preTime(end) preTime(end)],YL,'Color','k');
        ...
        for c=1:length(data_channel)
            ...
            plot(D.channel(c).peakTime-1,D.channel(c).peak,'R*');
            plot(D.channel(c).troughTime-1,D.channel(c).trough,'G*');
                  ...
                  switch button % deal with clicks
                    ...
                    case 104    % 'h' key pressed; bring up "help" dialog:
                      uiwait(helpdlg({'List of GUI commands',...
                                     ...
                                     'GUI Help'));
                    case {44,60}% ,/< key pressed; go back one trial
                      if f>1
                          skipSwitch = -1;
                          skipFlag = 1;
                          break;
                      end
                    case {46,62}% ./> key pressed; go forward one trial but only if the current trial is scored
                    ...
                    otherwise   % display what key was actually pressed; for debugging
                      disp(button);
                  end
                end
            ...
            % get the manually updated results
            D.channel(c).amplitude = D.channel(c).peak-D.channel(c).trough;
            M.channel(c).isGood = D.channel(c).isGood;
...
if nargout == 0
    ...
    if manual
        yesno = questdlg({'Do you want to save your work?', ...
                      ...
                      'base workspace.'},'Save','Yes','No','Yes');
    else
        yesno = 'Yes';
    end
