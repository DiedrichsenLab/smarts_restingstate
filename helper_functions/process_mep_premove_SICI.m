function varargout = process_mep_premove_SICI(dd,flags);
...
while i <= numFrames
    ...
    for c=1:length(data_channel)
        ...
        if c==1
            ...
            t=title('Entire moving trace (press H for Help)');
        else
            t=title('Entire non-moving trace');
        end
    ...
    for c=1:length(data_channel)
        ...
        if ~isfield(D.channel(c),'isGood') || isempty(D.channel(c).isGood)
            if c==1
                t=title(sprintf('Moving side %d of %d; State: %d',f,dd.frames,dd.frameinfo(f).state));
            else
                t=title(sprintf('Non-moving side %d of %d; State: %d',f,dd.frames,dd.frameinfo(f).state));
            end
        else
            if D.channel(c).isGood
                if c==1
                    t=title(sprintf('Moving side %d of %d; State: %d;    Good',f,dd.frames,dd.frameinfo(f).state));
                else
                    t=title(sprintf('Non-moving %d of %d; State: %d;    Good',f,dd.frames,dd.frameinfo(f).state));
                end
            else
                if c==1
                    t=title(sprintf('Moving side %d of %d; State: %d;    Bad',f,dd.frames,dd.frameinfo(f).state));
                else
                    t=title(sprintf('Non-moving side %d of %d; State: %d;    Bad',f,dd.frames,dd.frameinfo(f).state));
                end
            end
        end
            ...
            if ~isfield(D.channel(c),'isGood') || isempty(D.channel(c).isGood)
                ...
                if D.channel(c).isGood
                    if c==1
                        ...
                        t=title(sprintf('Non-moving side %d of %d; State: %d;    Good',f,dd.frames,dd.frameinfo(f).state));
                    end
                else
                    if c==1
                        t=title(sprintf('Moving side %d of %d; State: %d;    Bad',f,dd.frames,dd.frameinfo(f).state));
